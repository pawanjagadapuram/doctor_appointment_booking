...
const MyAppointments = () => {
    ...
    const getUserAppointments = async () => {
        try {
            const { data } = await axios.get(backendUrl + '/api/user/appointments', {Authorization: `Bearer ${token}`})
    ...
    const cancelAppointment = async (appointmentId) => {
        try {
            const {data} = await axios.post(backendUrl + '/api/user/cancel-appointment', {appointmentId}, {Authorization: `Bearer ${token}`}) 
            if(data.success){
    ...
    const initPay = (order) => {
        const options = {
            ...
            handler: async (response) => {
                ...
                try {
                    const {data} = await axios.post(backendUrl + '/api/user/verify-razorpay', response, {Authorization: `Bearer ${token}`})
                    if(data.success){
                        getUserAppointments()
    ...
    const appointmentRazorpay = async (appointmentId) => {
        try {
            const {data} = await axios.post(backendUrl + '/api/user/payment-razorpay', {appointmentId}, {Authorization: `Bearer ${token}`})
            if(data.success){
                initPay(data.order)
            }
        } catch (error) {
